#include "Slime.h"

#include "Engine/Animation.h"
#include "Engine/StaticMesh.h"
#include "Engine/World.h"
#include "HexaGame/Paths.h"
#include "HexaGame/Database/Items.h"

Slime::Slime()
{
    declared_height = 2;
    basic_movement_speed = 1.25f;
    
    step_animation = { Animation::load_from_file(RESOURCES_ANIMATIONS + "slime_step.anim"), false };
}

void Slime::on_start()
{
    Character::on_start();
    
    //set_mesh(StaticMesh::load_file_obj(RESOURCES_HEXA_MESHES_CHARACTERS + "slime.obj"));
    //get_material_instance()->set_param_value("texture", Texture::load_png(RESOURCES_HEXA_TEXTURES_CHARACTERS + "slime.png"));
}

void Slime::on_tick(float delta_time)
{
    Character::on_tick(delta_time);
}
