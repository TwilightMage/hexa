...
//-----------------------------------------------------------------------
DLight::DLight(MaterialGenerator *sys, Light* parentLight):
...
//-----------------------------------------------------------------------
bool DLight::isCameraInsideLight(Camera* camera)
{
    switch (mParentLight->getType())
    {
    case Light::LT_DIRECTIONAL:
    ...
    case Light::LT_POINT:
        {
            Real distanceFromLight = camera->getDerivedPosition()
    ...
    case Light::LT_SPOTLIGHT:
        {
            Vector3 lightPos = mParentLight->getDerivedPosition();
            Vector3 lightDir = mParentLight->getDerivedDirection();
            Radian attAngle = mParentLight->getSpotlightOuterAngle();
        
        //Extend the analytic cone's radius by the near clip range by moving its tip accordingly.
        //Some trigonometry needed here.
            Vector3 clipRangeFix = -lightDir * (camera->getNearClipDistance() / Math::Tan(attAngle/2));
        lightPos = lightPos + clipRangeFix;

            Vector3 lightToCamDir = camera->getDerivedPosition() - lightPos;
            Real distanceFromLight = lightToCamDir.normalise();

            Real cosAngle = lightToCamDir.dotProduct(lightDir);
...
//-----------------------------------------------------------------------
void DLight::updateFromCamera(Camera* camera)
{
    //Set shader params
    const MaterialPtr& mat = getMaterial();
    if (!mat->isLoaded()) 
    {
        mat->load();
    }
    ...
    for (unsigned short i=0; i<tech->getNumPasses(); i++) 
    {
        ...
        if (mParentLight->getType() == Light::LT_DIRECTIONAL)
        {
        ...
        else
        {
            ...
            if (isCameraInsideLight(camera))
            {
                ...
                pass->setDepthFunction(CMPF_GREATER_EQUAL);
            }
            else
            {
                pass->setCullingMode(CULL_CLOCKWISE);
