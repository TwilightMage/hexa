        ...
        const Renderable* rend)
{
    ...
    for (unsigned short i=0; i<originalTechnique->getNumPasses(); i++)
    {
        ...
        const MaterialPtr& templateMat = mMaterialGenerator.getMaterial(perm);
...
bool GBufferSchemeHandler::checkNormalMap(
    TextureUnitState* tus, PassProperties& props)
{
    bool isNormal = false;
    String lowerCaseAlias = tus->getName();
    StringUtil::toLowerCase(lowerCaseAlias);
    ...
    Pass* pass, unsigned short lodIndex, const Renderable* rend)
{
    ...
    //Check transparency
    if (pass->getDestBlendFactor() != SBF_ZERO)
    {
