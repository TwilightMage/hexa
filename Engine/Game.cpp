...
FORCEINLINE Name name_instancing(const Name& name) { return Name(name.to_string() + "_instancing"); } 
...
Shared<Texture> Game::load_texture(const ModuleAssetID& id)
{
	if (!id.is_valid())
	{
		print_error("Texture", "Invalid id %s, unable to load texture", id.to_string().c());
		return nullptr;
	}
	...
	if (get_stage() <= GameStage::Loading) print_warning("Texture", "Texture loading outside of loading stage is not recommended! Texture id: %s", id.to_string().c());

	const auto path = id.evaluate_asset_path().with_extension(".png");

	if (const auto result = load_texture_path(path, id.asset_name, id.module_name))
	{
		result->id_ = id;
...
Shared<Texture> Game::create_texture(const Array2D<Color>& pixels, const ModuleAssetID& id)
{
	if (!id.is_valid())
	{
		print_error("Texture", "Invalid id %s, unable to create texture", id.to_string().c());
		return nullptr;
	}
	...
	if (result = instance_->textures_.find_or_default(id))
	{
		...
		print_warning("Texture", "Texture %s was rewritten manually", id.to_string().c());
	}
	else
	{
		result = MakeShared<Texture>();
		result->ogre_texture_ = Ogre::TextureManager::getSingleton().loadImage(id.asset_name.c(), id.module_name.c(), ogre_image);
		result->id_ = id;
	...
	if (get_stage() <= GameStage::Loading) print_warning("Texture", "Texture creation outside of loading stage is not recommended! Texture path: %s", id.to_string().c());
...
{
	if (!CheckError(id.is_valid(), "Material", "Invalid id %s, unable to load material", id.to_string().c())) return nullptr;

	if (const auto slot = instance_->materials_.find_or_default(id)) return slot;
	
	Check(get_stage() <= GameStage::Loading, "Material", "material loading outside of loading stage is not recommended! Material id: %s", id.to_string().c());

	const auto path = id.evaluate_asset_path().with_extension(".mat");

	if (!CheckError(path.exists(), "Texture", "Failed to load texture %s, file does not exist", id.to_string().c())) return nullptr;
	
	Compound::Object material_json;
	if (!CheckError(Compound::Converters::JSON().try_parse_value(File::read_file(path), material_json), "Material", "Failed to read material %s info", id.to_string().c())) return nullptr;

	const auto vertex_program_name = ModuleAssetID(material_json.get_string("vertex"), id.module_name);
	const auto fragment_program_name = ModuleAssetID(material_json.get_string("fragment"), id.module_name);

	if (!CheckError(vertex_program_name.is_valid(), "Material", "Unable to load material %s because vertex program is not specified", id.to_string().c())) return nullptr;
	if (!CheckError(fragment_program_name.is_valid(), "Material", "Unable to load material %s because fragment program is not specified", id.to_string().c())) return nullptr;

	bool vertex_instancing;
	bool fragment_instancing;
	
	if (!CheckError(load_shader_program(vertex_program_name, vertex_instancing), "Material", "Unable to load material %s because of vertex shader loading fail")) return nullptr;
	if (!CheckError(load_shader_program(fragment_program_name, fragment_instancing), "Material", "Unable to load material %s because of fragment shader loading fail")) return nullptr;

	Shared<Ogre::Material> ogre_material = nullptr;
	Shared<Ogre::Material> ogre_material_instancing = nullptr;

	for (uint i = 0; i < (vertex_instancing || fragment_instancing ? 2 : 1); i++)
	{
		const bool inst_pass = i == 1;

		auto& mat = inst_pass ? ogre_material_instancing : ogre_material;
		mat = Ogre::MaterialManager::getSingleton().create(inst_pass ? name_instancing(id.asset_name).c() : id.asset_name.c(), id.module_name.c());
        ...
        pass->setVertexProgram(vertex_instancing && inst_pass ? name_instancing(vertex_program_name.asset_name).c() : vertex_program_name.asset_name.c());
        pass->setFragmentProgram(fragment_instancing && inst_pass ? name_instancing(fragment_program_name.asset_name).c() : fragment_program_name.asset_name.c());
        ...
        for (const auto& texture : material_json.get_array("textures"))
        {
        	if (texture.get_type() == Compound::Type::Object)
        	{
        		...
        		{
        			if (const auto default_texture = load_texture(ModuleAssetID(param_default->get_string(), id.module_name)))
        			{
        		...
        		if (const auto param_filtering = texture_data.find("filtering"))
        		{
        			...
        			const auto filter = filters.find_or_default(param_filtering->get_string(), Ogre::TextureFilterOptions::TFO_NONE);
...
Shared<Material> Game::clone_material(const Shared<Material>& material, const ModuleAssetID& new_id)
{
	if (material == nullptr) return nullptr;

	const ModuleAssetID actual_new_id = ModuleAssetID(new_id.module_name.is_valid() ? new_id.module_name : material->get_id().module_name, new_id.asset_name.is_valid() ? new_id.asset_name : Name(material->get_id().asset_name.to_string() + "_clone"));
	auto result = MakeShared<Material>();
	result->ogre_material_ = material->ogre_material_->clone(Ogre::String(actual_new_id.asset_name.c()), Ogre::String(actual_new_id.module_name.c()));
...
{
	if (instance_->get_module_name() == module_name) return instance_;
	
	for (const auto& mod : instance_->mods_)
	{
		if (mod->get_module_name() == module_name) return mod;
	}
...
void Game::setup()
{
	...
	Ogre::CompositorManager::getSingleton().registerCustomCompositionPass("DeferredLight", new DeferredLightCompositionPass());
}
	...
	if (args.length() >= 2 && args[1] == "comp")
	{
		if (args.length() >= 3)
		{
			...
			if (args.length() >= 4)
			{
				...
				const Path out = (args.length() >= 5) ? args[4] : src.parent + "/" + src.filename + ".comp";
			
				if (src.exists())
				{
					Shared<Compound::Converter> converter = nullptr;

					if (converter_name == "json") converter = MakeShared<Compound::Converters::JSON>();
					//else if (converter_name == "yaml") converter = MakeShared<Compound::Converters::YAML>();
					//else if (converter_name == "xml") converter = MakeShared<Compound::Converters::XML>();
					...
					if (converter)
					{
						...
						{
							auto os = std::ofstream(out.get_absolute_string().c(), std::ios::binary);
							StreamUtils::write<Compound::Object>(os, json);
							os.close();

							verbose("comp", "Conversion done!");
						}
						else print_error("comp", "Provided file contains broken compound value");
					}
					else print_error("comp", "Converter %s is not a valid converter", converter_name.c());
				}
				else print_error("comp", "Unable to read from provided source file");
			}
			else print_error("comp", "Source file path is required");
		}
		else print_error("comp", "Source format is required");
...
void Game::initialization_stage()
{
	...
	File::write_file(settings_path, converter.format_value(settings_->write_settings()));
...
void Game::load_tables(const Path& path)
{
	...
	{
		valid = Compound::Converters::JSON().try_parse_value(File::read_file(path), import_data);
	}
	else if (path.extension == ".bdb")
	{
		std::ifstream fin(path.get_absolute_string().c(), std::ios::binary | std::ios::in);
	...
	for (const auto& table_data : import_data)
	{
		...
		{
			print_error("Database", "Table named %s was not registered (declared in file %s)", table_data->key.c(), path.get_absolute_string().c());
		}
	}

	//std::ofstream of((path.get_absolute_string() + ".bdb").c(), std::ios::out | std::ios::binary);
    //
...
Shared<Texture> Game::load_texture_path(const Path& path, const Name& name, const Name& module_name)
{
	...
	const auto ogre_texture = Ogre::TextureManager::getSingleton().loadImage(name.c(), module_name.c(), image);
	...
	if (pixels.length() > 0)
	{
		...
		{
			Ogre::Image ogre_image(Ogre::PF_R5G6B5, tex_width, tex_height);

			for (uint x = 0; x < tex_width; x++)
			{
				for (uint y = 0; y < tex_height; y++)
				{
					ogre_image.setColourAt(Ogre::ColourValue((byte*)&pixels[y * tex_width + x]), x, y, 0);
				}
			}
			
			
			auto result = MakeShared<Texture>();
			result->ogre_texture_ = Ogre::TextureManager::getSingleton().load
...
{
	if (Ogre::GpuProgramManager::getSingleton().getResourceByName(id.asset_name.c(), id.module_name.c())) return true;
	...
	if (definition_path.exists())
	{
		...
		if (Compound::Converters::JSON().try_parse_value(File::read_file(definition_path), program_json))
		{
			const auto source_name = ModuleAssetID(program_json.get_string("source"), id.module_name);
			if (source_name.is_valid())
			{
				const auto source_path = source_name.evaluate_asset_path().with_extension(".hlsl");
				if (source_path.exists())
				{
					...
					if (program_type_index > -1)
					{
						...
						const static Map<String, Ogre::GpuProgramParameters::AutoConstantType> param_types = {
						...
						for (uint i = 0; i < (instancing ? 2 : 1); i++)
						{
							...
							const auto program = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(inst_pass ? name_instancing(id.asset_name).c() : id.asset_name.c(), id.module_name.c(), "hlsl", Ogre::GpuProgramType(program_type_index));
							programs[i] = program;
							program->setSource(File::read_file(source_path).c()); // TODO: Fix ogre to accept global path
							...
							for (auto& param : program_json.get_object(inst_pass ? "instancing_params" : "params"))
							{
								...
								{
									print_warning("Shader Program", "Parameter %s in shader program %s has invalid type: %s", param->key.c(), id.to_string().c(), param_type_name.c());
								}
							}
							...
							{
								print_error("Shader Program", "Failed to create shader program %s, aborting", program->getName().c_str());
								success = false;
