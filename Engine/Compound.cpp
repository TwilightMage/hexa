...
{
    String desired = "[" + type_names[(char)desired_type] + "]";
    if (desired_binary_size > 0) desired += "<" + String::make(desired_binary_size) + " chars>";

    String actual = "[" + type_names[(char)actual_type] + "]";
...
String Compound::Converters::JSON::format(const Value& val, uint depth) const
{
    ...
    case Type::Array:
        ...
        {
            result = "[" + new_line() + tab_offset(depth + 1) + String::join<String>(val.get_array().select<String>([&](const Value& val) -> String{ return format(val, depth + 1); }), (separate_with_new_line ? "," : ", ") + new_line() + tab_offset(depth + 1)) + new_line() + tab_offset(depth) + "]";
        }
        break;
    case Type::Object:
...
Compound::Value Compound::Converters::JSON::read_value() const
{
    ...
    throw new ParseException(reader.get_line(), reader.get_column());
}
...
void Compound::Converters::JSON::read_null() const
{
    ...
    {
        throw new ParseException(reader.get_line(), reader.get_column(), token);
    }
}

String Compound::Converters::JSON::read_string() const
{
    if (reader.get() != '"') throw new ParseException(reader.get_line(), reader.get_column());
    ...
    while(true)
    {
        if (reader.is_done()) throw new ParseException(reader.get_line(), reader.get_column(), token);
...
double Compound::Converters::JSON::read_number() const
{
    ...
    return String::parse<int64>(token);
}
...
Compound::Object Compound::Converters::JSON::read_object() const
{
    ...
    while(true)
    {
        if (reader.is_done()) throw new ParseException(reader.get_line(), reader.get_column());
...
void Compound::Value::read_from_stream(std::istream& stream)
{
    ...
    case Type::Int32:
        ...
        break;
    case Type::Int64:
        set_int32(StreamUtils::read<int64>(stream));
        break;
    case Type::Bool:
    ...
    case Type::Object:
        ...
        break;
    case Type::BinaryData:
        uint64 size = StreamUtils::read<uint64>(stream);
...
{
    if (type != Type::String) throw new TypeMismatchException(Type::String, type);
...
{
    if (type != Type::Char) throw new TypeMismatchException(Type::Char, type);
...
{
    if (type != Type::Short) throw new TypeMismatchException(Type::Short, type);
...
{
    if (type != Type::Int32) throw new TypeMismatchException(Type::Int32, type);
...
{
    if (type != Type::Int64) throw new TypeMismatchException(Type::Int64, type);
...
{
    if (type != Type::Bool) throw new TypeMismatchException(Type::Bool, type);
...
{
    if (type != Type::Float) throw new TypeMismatchException(Type::Float, type);
...
{
    if (type != Type::Double) throw new TypeMismatchException(Type::Double, type);
...
{
    if (type != Type::Array) throw new TypeMismatchException(Type::Array, type);
...
{
    if (type != Type::Object) throw new TypeMismatchException(Type::Object, type);
