...
void DialoguePanel::on_parent_size_changed()
{
    ...
    {
        set_size(Vector2(min(1080.0f, parent->get_size().x - 20), 200));
...
{    
    ...
    close_button_ = MakeShared<ButtonImage>(Texture::load_png("resources/hexagame/textures/ui/cross.png"));
    close_button_->set_size(Vector2(32, 32));
    close_button_->set_z(0.1f);
    panel_->add_child(close_button_);
...
void DialoguePanel::update_geometry()
{
    if (is_started_construction())
    {
        ...
        close_button_->set_position(Vector2(get_size().x - 52, 20));
    }
}
